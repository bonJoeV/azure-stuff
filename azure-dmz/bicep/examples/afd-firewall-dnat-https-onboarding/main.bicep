...
@description('AFD WAF mode: Detection | Prevention')
@allowed([
  'Detection'
  'Prevention'
])
...
@description('Optional custom domain host name to create for AFD (e.g., app.contoso.com). No DNS records are created by this template.')
param customDomainHostName string = ''

@description('TLS cert type for AFD custom domain.')
@allowed([
  'ManagedCertificate'
  'CustomerCertificate'
])
...
@description('AFD secret resource ID (Microsoft.Cdn/profiles/secrets) when using CustomerCertificate.')
...
// DNAT: Firewall public IP:443 -> App Service Private Endpoint IP:443
...
module afdWaf '../../modules/ingress/afdWafPolicy.bicep' = {
  name: 'afd-waf-${environment}-${toLower(wafMode)}'
  scope: hubRg
  params: {
    environment: environment
    location: location
    policyBaseName: 'waf-afd'
    mode: wafMode
    tags: tags
  }
}

// Front Door (AFD) routes to the firewall public IP as an origin over HTTPS.
// Important: originHostHeader is set to the App Service hostname so SNI matches the backend certificate.
module afd '../../modules/ingress/frontDoor.bicep' = {
  ...
  params: {
    ...
    sampleOriginHttpsPort: 443
    sampleForwardingProtocol: 'HttpsOnly'
    sampleRouteHostName: app.outputs.appServiceDefaultHostName
    customDomainHostName: customDomainHostName
    customDomainCertificateType: customDomainCertificateType
    customDomainAfdSecretId: customDomainAfdSecretId
    tags: tags
  }
}

module afdWafAssoc '../../modules/ingress/afdWafSecurityPolicy.bicep' = {
  name: 'afd-waf-assoc-${environment}-${toLower(wafMode)}'
  scope: hubRg
  params: {
    frontDoorProfileId: afd.outputs.frontDoorProfileId
    afdEndpointId: afd.outputs.frontDoorEndpointId
    wafPolicyId: afdWaf.outputs.wafPolicyId
    securityPolicyName: 'sp-waf-${environment}-${toLower(wafMode)}'
    patternsToMatch: [
      '/*'
    ]
  }
...
output frontDoorEndpointHostName string = afd.outputs.frontDoorEndpointHostName
output wafPolicyId string = afdWaf.outputs.wafPolicyId
output customDomainId string = afd.outputs.customDomainId
output customDomainValidationToken string = afd.outputs.customDomainValidationToken
